...
void exportToJson(int algorithm_index) {
    std::ofstream out("timeline.json");
    out << "{\n  \"processes\": [\n";
    for (int i = 0; i < process_count; i++) {
        out << "    {\n";
        out << "      \"name\": \"" << getProcessName(processes[i]) << "\",\n";
        out << "      \"timeline\": [";
        for (int j = 0; j < last_instant; j++) {
            out << "\"" << timeline[j][i] << "\"";
            if (j != last_instant - 1) out << ", ";
        }
        out << "]\n    }";
        if (i != process_count - 1) out << ",";
        out << "\n";
    }
    out << "  ]\n}";
    out.close();
...
int main() {
    ...
    for (int idx = 0; idx < (int)algorithms.size(); idx++) {
        ...
        if (operation == TRACE) {
            printTimeline(idx);
            exportToJson(idx);  // ? safely inside if block
        } else if (operation == SHOW_STATISTICS) {
            printStats(idx);
            exportToJson(idx);  // ? safe for stats too
        }
